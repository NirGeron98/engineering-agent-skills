...
- A single slow query needs diagnosis; use `.claude/skills/query-performance-review`.
- A live migration's safety (locks, backfill) is the concern, not index design itself; use `.claude/skills/schema-migration-safety-review`.
- The bottleneck is broader than indexing (CPU, IO, concurrency); use `.claude/skills/throughput-bottleneck-triage`.

## Composition and handoff rules

- Defer slow single-query diagnosis to `.claude/skills/query-performance-review`.
- Defer live migration safety to `.claude/skills/schema-migration-safety-review`.
- Defer broad performance triage to `.claude/skills/throughput-bottleneck-triage`.
