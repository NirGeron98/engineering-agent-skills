...
- The reported issue is an active data inconsistency incident, not a caching design question; use `.claude/skills/data-consistency-incident-diagnosis`.
- The question is which code path to optimize based on profiling evidence, not whether to cache; use `.claude/skills/hot-path-profiling-review`.
- The decision is a broad system architecture tradeoff beyond caching itself; use `.claude/skills/architecture-review`.

## Composition and handoff rules

- Defer data consistency incidents to `.claude/skills/data-consistency-incident-diagnosis`.
- Defer hot path profiling to `.claude/skills/hot-path-profiling-review`.
- Defer architecture-level tradeoffs to `.claude/skills/architecture-review`.
