...
- The bottleneck is clearly a specific database query, not general code path time; use `.claude/skills/query-performance-review`.
- Only trace evidence exists and profiling is not the tool in play; use `.claude/skills/trace-based-latency-diagnosis`.
- The slowness is in frontend rendering/load, not backend; use `.claude/skills/frontend-performance-review`.

## Composition and handoff rules

- Defer database-specific bottlenecks to `.claude/skills/query-performance-review`.
- Defer trace-only diagnosis to `.claude/skills/trace-based-latency-diagnosis`.
- Defer frontend performance to `.claude/skills/frontend-performance-review`.
