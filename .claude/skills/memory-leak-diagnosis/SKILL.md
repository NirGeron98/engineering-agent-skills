...
- The question is a Kubernetes/pod-level OOM policy or resource limit issue, not the application's retention path; use `.claude/skills/kubernetes-workload-debugging`.
- The issue is a general service bug unrelated to memory; use `.claude/skills/service-debugging`.
- The central question is whether to add/remove caching, not diagnosing a leak; use `.claude/skills/caching-strategy-review`.

## Composition and handoff rules

- Defer Kubernetes OOM/pod-level diagnosis to `.claude/skills/kubernetes-workload-debugging`.
- Defer general service debugging to `.claude/skills/service-debugging`.
- Defer cache design to `.claude/skills/caching-strategy-review` when caching is central.
