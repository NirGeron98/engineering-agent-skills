...
- The bug is not understood; use `.claude/skills/diagnose-bug` first.
- The user needs a broad coverage plan; use `.claude/skills/test-strategy`.
- Legacy behavior must be captured before refactor; use `.claude/skills/legacy-characterization-tests`.

## Composition and handoff rules

- Hand root-cause diagnosis to `.claude/skills/diagnose-bug` before writing tests when the cause is unknown.
- Hand broader test planning to `.claude/skills/test-strategy`.
- Hand legacy behavior capture to `.claude/skills/legacy-characterization-tests`.
