...
- Use this even if the eventual root cause may turn out to be a single service; only hand off to `.claude/skills/service-debugging` once evidence shows an isolated in-process bug with no cross-store divergence.

## When not to use

- The concern is data warehouse quality contracts, not operational/application data; use `.claude/skills/data-quality-contracts`.
- The root cause is clearly a single service's runtime bug, not a cross-store consistency question; use `.claude/skills/service-debugging`.
- Incident coordination/communication (not the data diagnosis itself) is the need; use `.claude/skills/incident-triage-runbook`.

## Composition and handoff rules

- Defer data warehouse quality contracts to `.claude/skills/data-quality-contracts`.
- Defer service runtime debugging to `.claude/skills/service-debugging`.
- Defer incident coordination to `.claude/skills/incident-triage-runbook`.
