...
- The incident is not security-specific (outage, deploy failure); use `.claude/skills/incident-triage-runbook`.
- The concern is a secret found but with no evidence of use; use `.claude/skills/secrets-exposure-review`.
- A durable runbook needs to be authored from this scoping; use `.claude/skills/runbook-writer`.

## Composition and handoff rules

- Defer general incident coordination to `.claude/skills/incident-triage-runbook` when it is not security-specific.
- Defer secrets-specific containment to `.claude/skills/secrets-exposure-review`.
- Defer runbook drafting to `.claude/skills/runbook-writer`.
