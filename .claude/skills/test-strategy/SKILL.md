...
- The user wants to implement the change test-first; use `.claude/skills/tdd-workflow`.
- A test is flaky or failing unpredictably; use `.claude/skills/flaky-test-debugging`.
- An E2E/browser test failed and needs diagnosis; use `.claude/skills/e2e-test-debugging`.
- The feature itself is still vague; use `.claude/skills/task-breakdown`.

## Composition and handoff rules

- Hand test-first implementation slices to `.claude/skills/tdd-workflow`.
- Hand flaky failure diagnosis to `.claude/skills/flaky-test-debugging`.
- Hand E2E failure investigation to `.claude/skills/e2e-test-debugging`.
- Hand feature planning to `.claude/skills/task-breakdown`.
