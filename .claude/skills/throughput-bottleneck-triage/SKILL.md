...
- The bottleneck is clearly a specific database query; use `.claude/skills/query-performance-review`.
- The concern is only whether alerts/metrics for throughput are well-designed, not diagnosing the bottleneck itself; use `.claude/skills/metrics-and-alerts-review`.
- The question is release/rollout readiness for a scaling change, not the diagnosis itself; use `.claude/skills/release-and-rollback-readiness`.

## Composition and handoff rules

- Defer query-specific bottlenecks to `.claude/skills/query-performance-review`.
- Defer alert/metrics quality to `.claude/skills/metrics-and-alerts-review`.
- Defer infrastructure rollout readiness to `.claude/skills/release-and-rollback-readiness`.
