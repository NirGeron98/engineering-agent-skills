...
- The issue is primarily CI pipeline configuration; use `skills/devops-infrastructure/ci-cd-debugging`.
- The failure is now reproducible as a code defect; use `skills/debugging/diagnose-bug`.
- The failure is E2E-specific and needs browser artifacts; use `skills/testing/e2e-test-debugging`.

## Composition and handoff rules

- Hand CI pipeline environment issues to `skills/devops-infrastructure/ci-cd-debugging`.
- Hand reproducible code defects to `skills/debugging/diagnose-bug`.
- Hand E2E-specific evidence handling to `skills/testing/e2e-test-debugging`.
