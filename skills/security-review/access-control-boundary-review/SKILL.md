...
- The concern is user-facing auth flow behavior (login, session, token issuance); use `skills/api-and-backend/auth-flow-review`.
- The concern is cloud provider IAM debugging (AWS/GCP/Azure roles and policies not resolving as expected); use `skills/devops-infrastructure/iam-permissions-debugging`.
- The concern is Terraform plan mechanics for IAM resources; use `skills/devops-infrastructure/terraform-plan-review`.

**Rule of thumb**: use this skill for resource ownership, tenant boundaries, service-to-service permissions, admin/user boundaries, or privilege boundaries. If the review centers on login, session management, token issuance, authentication flow, password reset, or OAuth/OIDC behavior, use `skills/api-and-backend/auth-flow-review` instead. If it centers on cloud IAM failure/debugging, use `skills/devops-infrastructure/iam-permissions-debugging` instead.

## Composition and handoff rules

- Defer user-facing auth flow behavior to `skills/api-and-backend/auth-flow-review`.
- Defer cloud IAM debugging to `skills/devops-infrastructure/iam-permissions-debugging`.
- Defer Terraform plan mechanics to `skills/devops-infrastructure/terraform-plan-review`.
