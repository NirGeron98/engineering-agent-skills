...
- The symptom is a single slow query without contention/pool signals; use `skills/database/query-performance-review`.
- The root cause is clearly general application logic, not DB connections/locks; use `skills/api-and-backend/service-debugging`.
- Incident coordination/communication (not the technical diagnosis) is the need; use `skills/devops-infrastructure/incident-triage-runbook`.

## Composition and handoff rules

- Defer general service debugging to `skills/api-and-backend/service-debugging`.
- Defer query plan tuning to `skills/database/query-performance-review`.
- Defer incident response coordination to `skills/devops-infrastructure/incident-triage-runbook`.
