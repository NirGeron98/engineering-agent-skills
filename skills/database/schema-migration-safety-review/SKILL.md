...
- The question is which indexes to add for query patterns, not migration mechanics; use `skills/database/index-strategy-review`.
- General release/rollback process (not schema-specific) is the concern; use `skills/devops-infrastructure/release-and-rollback-readiness`.

## Composition and handoff rules

- Defer general release readiness to `skills/devops-infrastructure/release-and-rollback-readiness`.
- Defer technical architecture decisions to `skills/architecture/architecture-decision-record`.
